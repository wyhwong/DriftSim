...
def compute_match(time_series_1, time_series_2, psd, config):
    match = pycbc.filter.matchedfilter.match(time_series_1, time_series_2, psd=psd,
                                             **config)
    squared_opt_snr_1 = pycbc.filter.matchedfilter.sigmasq(time_series_1, psd=psd,
                                                           **config)
    opt_snr_1 = np.sqrt(squared_opt_snr_1)
    squared_opt_snr_2 = pycbc.filter.matchedfilter.sigmasq(time_series_2, psd=psd,
                                                           **config)
